data {
  ...
  // CHANGED: Age-specific priors for detection probability
  real<lower=0, upper=1> prior_rho[A];  // prior means for each age's detection
  real<lower=0> prior_sd_rho;          // common sd for detection priors
}

parameters {
  // Age-specific initial infected count, with bounds and informative prior
  real<lower=10, upper=1000> I0[A];
  
  // Overall time-varying baseline transmission rate (applied to all ages)
  real<lower=0, upper=6> base_beta[T];
  
  // CHANGED: Instead of a single real<lower=0,upper=1> rho, 
  // we have an array of raw detection parameters in [0,1].
  real<lower=0, upper=1> rho_raw[A];
...
transformed parameters {
  ...
  // CHANGED: We define the actual detection probability rho[a].
  // It's the same dimension as rho_raw, but we can transform if needed.
  real<lower=0, upper=1> rho[A];
  ...
  // CHANGED: Assign rho[a] from rho_raw[a]. If you want a more complex transform, do it here.
  for (a in 1:A) {
    rho[a] = rho_raw[a];
  }
}
...
model {
  ...
  // CHANGED: Age-specific detection priors. 
  // Here we use a truncated normal( prior_rho[a], prior_sd_rho ) T[0,1].
  for (a in 1:A) {
    rho_raw[a] ~ normal(prior_rho[a], prior_sd_rho) T[0,1];
  }
  ...
  // --- 3) Observation Model ---
  // CHANGED: We now use rho[a] instead of a single rho.
  for (t in 1:T) {
    for (a in 1:A) {
      real mu = rho[a] * (I[a, t] + IV[a, t]);
...
generated quantities {
  ...
  for (a in 1:A) {
    ...
    // CHANGED: We now use rho[a] for detection.
    age_stratified_cases[a, 1] = rho[a] * (I_pred[a, 1] + IV_pred[a, 1]);
  }
  ...
  for (t in 2:T) {
    ...
    for (a in 1:A) {
      ...
      age_stratified_cases[a, t] = rho[a] * (I_pred[a, t] + IV_pred[a, t]);
    }
  ...
  for (t in 1:T) {
    ...
    pred_cases[t] = rho[1] * 0; // placeholder; see below
    aggregated_infections[t] = total_infections_t;
  }
  // If you want an aggregated predicted detection, you might do:
  // pred_cases[t] = sum_{a} rho[a] * (I_pred[a,t] + IV_pred[a,t]) ?
  // But that is not always correct if detection is purely age-specific. 
  // You could define a separate approach for aggregated "detected" cases if needed.
}
