...
parameters {
  ...
  real<lower=17,  upper=18>  shape;  // Overdispersion
...
transformed parameters {
  ...
  for (t in 1:T) {
    beta[t] = base_beta[t] * (0.5 + 0.5 * indexP_normalised[t]);
  }
}
...
model {
  ...
  // base_beta[t]
  for (t in 1:T) {
    base_beta[t] ~ lognormal(-1, 0.5);  // Example prior
  }
  ...
  // overdispersion
  shape ~ uniform(17, 18);  // originally (17,18)
