data {
  ...
  real<lower=0, upper=1> eta;               // Desired vaccine coverage
...
parameters {
  ...
  real<lower=0, upper=6> beta_baseline[Y];
  real<lower=-1, upper=1> seasonal_effect[T];
...
transformed parameters {
  ...
  for (t in 1:T) {
    int year = (t - 1) / 52 + 1;
    real seasonal_multiplier = exp(seasonal_effect[t]);
    beta[t] = beta_baseline[year] * seasonal_multiplier * (1 + 0.5 * indexP_normalised[t]);
  }
}

model {
  for (y in 1:Y)
    beta_baseline[y] ~ lognormal(-1, 0.8);

  seasonal_effect[1] ~ normal(0, 0.2);
  for (t in 2:T)
    seasonal_effect[t] ~ normal(seasonal_effect[t-1], sigma_season);

  sigma_season ~ exponential(0.5);
  rho ~ beta(30, 70);
...

  {
    ...
    for (t in 2:T) {
      ...
      for (a in 1:A) total_infected += I[a,t-1] + IV[a,t-1];
      real pop_total = 0;
      for (a in 1:A) pop_total += N[a];
      real phi = beta[t-1] * total_infected / pop_total;

      for (a in 1:A) {
        real vacc_effect = ((t >= delay) && (t <= delay+6) && (vaccine_target_age[a] == 1)) ? vaccination_rate : 0;
    ...
    for (t in 1:T)
      for (a in 1:A)
        observed_cases_by_age[a,t] ~ neg_binomial_2(rho * (I[a,t] + IV[a,t]), shape);
  }
}
...
generated quantities {
  ...
  {
    real susceptible_unvacc = 0;
    real susceptible_vacc = 0;
    for (a in 1:A) {
      susceptible_unvacc += S_pred[a,1];
      susceptible_vacc += V_pred[a,1];
    }
    R_eff[1] = (beta[1] / gamma) * ((susceptible_unvacc + (1 - VE_block) * susceptible_vacc) / pop_total);
  }
