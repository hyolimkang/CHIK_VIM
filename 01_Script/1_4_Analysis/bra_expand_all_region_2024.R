bra_all_ages_region_24 <- read_excel("00_Data/0_1_Raw/epi_report_plisa.xlsx", 
                                  sheet = "bra_region_age_week_2024")
...
bra_all_ages_region_24_sum <- bra_all_ages_region_24 %>% group_by(region, week) %>% # use epi_week when only using 2022 data
  summarise(
    epi_week  = first(week),
    tot_cases = sum(cases)
  )

bra_all_sum_24 <- bra_all_ages_region_24_sum %>% group_by(week) %>% # use epi_week when only using 2022 data
  summarise(
    epi_week  = first(week),
    tot_cases = sum(tot_cases)
  )

colnames(bra_all_sum_24)[1] <- "Week"
colnames(bra_all_sum_24)[3] <- "Observed"
...
# split into regions 
bra_sum_bh_24 <- bra_all_ages_region_24_sum %>% filter(region == "Bahia" & week <=26)
bra_sum_ce_24 <- bra_all_ages_region_24_sum %>% filter(region == "Ceará")
bra_sum_mg_24 <- bra_all_ages_region_24_sum %>% filter(region == "Minas Gerais"& week <=30)
bra_sum_pn_24 <- bra_all_ages_region_24_sum %>% filter(region == "Pernambuco")
bra_sum_go_24 <- bra_all_ages_region_24_sum %>% filter(region == "Goiás")
bra_sum_mt_24 <- bra_all_ages_region_24_sum %>% filter(region == "Mato Grosso"& week <=40)
bra_sum_ms_24 <- bra_all_ages_region_24_sum %>% filter(region == "Mato Grosso do Sul"& week <=25)
bra_sum_sp_24 <- bra_all_ages_region_24_sum %>% filter(region == "São Paulo"& week <=35)
...
bra_all_ages_region_24$age_gr <- factor(bra_all_ages_region_24$age_gr, levels = age_gr_levels)

bra_expanded_region_24 <- bra_all_ages_region_24 %>%
...
bra_expanded_region_24 <- bra_expanded_region_24 %>%
  rowwise() %>%
...
# by region
bra_bh_24 <- bra_expanded_region_24 %>% filter(region == "Bahia", week <= 26)
bra_ce_24<- bra_expanded_region_24 %>% filter(region == "Ceará")
bra_mg_24 <- bra_expanded_region_24 %>% filter(region == "Minas Gerais", week <= 30) 
bra_pn_24 <- bra_expanded_region_24 %>% filter(region == "Pernambuco")
bra_go_24 <- bra_expanded_region_24 %>% filter(region == "Goiás")
bra_mt_24 <- bra_expanded_region_24 %>% filter(region == "Mato Grosso", week <= 40)
bra_ms_24 <- bra_expanded_region_24 %>% filter(region == "Mato Grosso do Sul", week <= 25)
bra_sp_24 <- bra_expanded_region_24 %>% filter(region == "São Paulo", week <= 35)
...
bra_bh_24$age_bin_label <- factor(bra_bh_24$age_bin_label, levels = model_age_bins)
...
bra_ce_24$age_bin_label <- factor(bra_ce_24$age_bin_label, levels = model_age_bins)
...
bra_mg_24$age_bin_label <- factor(bra_mg_24$age_bin_label, levels = model_age_bins)
...
bra_pn_24$age_bin_label <- factor(bra_pn_24$age_bin_label, levels = model_age_bins)
...
bra_go_24$age_bin_label <- factor(bra_go_24$age_bin_label, levels = model_age_bins)
...
bra_mt_24 <- bra_mt_24 %>%
  group_by(week, age_gr, region) %>%
  mutate(pop_total = sum(N_mt[age_bin])) %>%  # Sum only over the bins that the record covers
  ungroup() %>%
  mutate(cases_bin = cases * (N_mt[age_bin] / pop_total))
...
bra_mt_24$age_bin_label <- factor(bra_mt_24$age_bin_label, levels = model_age_bins)
...
bra_ms_24$age_bin_label <- factor(bra_ms_24$age_bin_label, levels = model_age_bins)
...
bra_sp_24$age_bin_label <- factor(bra_sp_24$age_bin_label, levels = model_age_bins)
...
ggplot(bra_sum_sp_24)+
  geom_line(aes(x= week, y = tot_cases))+
  theme_minimal()

combined_24_all <- bind_rows(
  bra_sum_bh_24, bra_sum_ce_24, bra_sum_mg_24,
  bra_sum_pn_24, bra_sum_mt_24, bra_sum_ms_24,
  bra_sum_go_24, bra_sum_sp_24
)

ggplot(combined_24_all)+
  geom_line(aes(x= week, y = tot_cases))+
  theme_minimal()+
  facet_wrap(~region)
...
combined_2024 <- combined_24_all %>% group_by(week) %>% summarise(
  tot_cases = sum(tot_cases)
)

ggplot(combined_2024)+
  geom_line(aes(x= week, y = tot_cases))+
  theme_light()+
