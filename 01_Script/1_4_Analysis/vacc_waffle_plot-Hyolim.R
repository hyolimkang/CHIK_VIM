...
waffle_weekly_data <- combined_vacc_alloc_summ %>%
  group_by(Scenario, Week, age_gr) %>%
  summarise(doses = sum(Vaccinated, na.rm = TRUE), .groups = "drop") %>%
  group_by(Scenario, Week) %>%
  mutate(prop = doses / sum(doses) * 100) %>%
  ungroup() %>%
  mutate(
    Scenario = factor(Scenario, levels = names(scenario_labels)),
    age_gr = factor(age_gr, levels = names(full_palette))
...
ggplot(waffle_weekly_data_subset, aes(values = prop, fill = age_gr)) +
