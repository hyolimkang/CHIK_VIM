...
# sample 
dom <- high_countries %>% filter(country == "Dominican Republic")

ggplot(dom[1:12,], aes(x = date, y = cases, color = group, group = country)) +
  geom_line(size = 1) +
  theme_minimal() +
  facet_wrap(~ group + country, scales = "free_y")  +  # Adjusts y-axis scale per country
  theme(legend.position = "none")

stan_model_age <- stan_model("01_Script/1_2_SIR_models/age_struc_vacc_scenario_dom.stan")


# according to cdc data (https://wwwnc.cdc.gov/eid/article/30/12/24-0824_article),
# San Pedro de Macorís and Espaillat reported chikungunya while others had 0 chik
N_dom <- as.matrix(dom_pop_transposed[,6] + dom_pop_transposed[,21])
N_dom <- as.matrix(dom_pop_transposed$tot_pop)

stan_data_prevacc <- list(
  T = length(dom[1:12,]$cases),
  A = 18,
  N = as.vector(N_dom), 
  observed_cases = dom[1:12,]$cases,
  r = rep(0,18),
  delay = 53,
  VE_block = 0,
  vaccination_rate = 0,
...
predicted_cases_df$Month <- as.Date(predicted_cases_df$Month)
observed_df$Month <- as.Date(observed_df$Month)
...
p <- ggplot() +
  ...
  geom_ribbon(data = predicted_cases_df, aes(x = Month, ymin = Lower, ymax = Upper), 
              fill = "blue", alpha = 0.2) +
  labs(title = "Epi-curve fitted to monthly DOM data (2014-15)",
       x = "Month", y = "Number of infectious individuals") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +  
  theme_minimal()


ggsave(filename = "02_Outputs/2_1_Figures/dom_fit.jpg", p, width = 11, height = 6, dpi = 1200)
